...
- Gateway/Auth: remove implicit unauthenticated startup fallback by defaulting unresolved auth to token mode, auto-generating/persisting `gateway.auth.token` at gateway startup when missing, and centralizing startup auth bootstrap logic shared by gateway/runtime/browser flows.
